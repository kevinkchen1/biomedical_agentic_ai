...
Enhanced with Interactive Query Explainer & Debugger
"""
...
from agents.graph_interface import GraphInterface  # noqa: E402
from agents.workflow_agent import WorkflowAgent  # noqa: E402
from agents.query_explainer import QueryExplainer  # noqa: E402
...
def initialize_agent():
    ...
    return workflow_agent, graph_interface, query_explainer


def display_query_explanation(explainer, cypher_query):
    """Display interactive query explanation."""
    explanation = explainer.explain_query(cypher_query)
    ...
    with col2:
        ...
        st.metric(
            ...
            complexity['level'],
            delta=None,
            help=complexity['description']
        )
    
    # Visual diagram
    st.markdown("### 🎨 Visual Pattern")
    st.code(explanation['visual'], language="text")
    ...
    for step in explanation['steps']:
        with st.expander(
            f"{step['icon']} Step {step['number']}: {step['operation']}", 
            expanded=(step['number'] <= 2)
        ):
            st.code(step['code'], language="cypher")
            st.markdown(f"**What this does:** {step['explanation']}")
    ...
    if explanation['warnings']:
        ...
        for warning in explanation['warnings']:
            if warning['severity'] == 'warning':
                st.warning(f"{warning['icon']} **{warning['type']}**: {warning['message']}\n\n"
                          f"💡 *Suggestion: {warning['suggestion']}*")
            else:
                st.info(f"{warning['icon']} **{warning['type']}**: {warning['message']}\n\n"
                       f"💡 *{warning['suggestion']}*")
    
    # Result size estimate
    st.markdown("### 📊 Expected Results")
    estimate = explanation['estimated_results']
    st.markdown(f"{estimate['icon']} **{estimate['estimate']}**  \n*{estimate['reasoning']}*")
...
def main_interface(workflow_agent, graph_interface, query_explainer):
    tab1, tab2, tab3 = st.tabs(["Concepts", "Try the Agent", "Explore Queries"])
    ...
    with tab2:
        ...
        if st.button("Run Workflow Agent", type="primary"):
            if question_input:
                ...
                # NEW: Query Explanation
                if result["cypher_query"]:
                    with st.expander("🔍 Understand This Query (Click to Expand)", expanded=False):
                        display_query_explanation(query_explainer, result["cypher_query"])
    ...
    with tab3:
        ...
        # NEW: Always show query explanation
        if query_text.strip():
            with st.expander("🔍 Query Explanation", expanded=True):
                display_query_explanation(query_explainer, query_text)
        ...
        if st.button("Execute Query"):
            if query_text.strip():
                try:
                    ...
                    st.error(f"Query error: {str(e)}")
                    st.info("Check the explanation above for potential issues!")
            else:
                st.warning("Please enter a query!")


def main():
    workflow_agent, graph_interface, query_explainer = initialize_agent()
    ...
    main_interface(workflow_agent, graph_interface, query_explainer)
